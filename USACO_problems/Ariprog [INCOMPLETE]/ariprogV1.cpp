	...
	for (int j = 1; j < 2*M*M/(N-1) + 1; j++)
		if ((N >= 4 && j % 2 == 0) || N < 4)
		{
			for (int i = 0; i < length - N + 1; i++)
			{
				...
				common_difference = j;
				if (validProg(bisquares, starting_number, common_difference, N))
				{
					cout << starting_number << " " << common_difference << endl;
					fout << starting_number << " " << common_difference << endl;
					count++;
//					fout << starting_number << " " << j << endl;
				}	
